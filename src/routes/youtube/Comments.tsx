...
const getRiskInfo = (score?: number): RiskInfo => {
  if (score === undefined) {
    return { label: "분석 없음", variant: "muted" };
  }
  if (score >= 0.75) {
    return { label: "위험도 높음", variant: "danger" };
  }
  if (score >= 0.4) {
    return { label: "주의 필요", variant: "warning" };
  }
  return { label: "안정", variant: "success" };
};
...
const YoutubeComments = () => {
  ...
  const handleThreadSelect = (thread: CommentThread) => {
    setSelectedThread(thread);
  };

  const handleClose = () => setSelectedThread(null);

  const renderThreadCard = (thread: CommentThread) => {
    const risk = getRiskInfo(thread.aiRiskScore);
    return (
      <button
        key={thread.id}
  ...
  const detailRisk = getRiskInfo(selectedThread?.aiRiskScore);
    ...
    <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
      <aside className="space-y-4">
        ...
        <div className="rounded-2xl border border-[var(--outline)] bg-[var(--surface-2)] p-4 shadow-sm">
          ...
          <p className="mt-1 text-xs text-[var(--muted)]">
            원하는 방식으로 댓글을 정렬하세요. OpenAI 에이전트는 위험도가 높은 스레드를 먼저 보여 줍니다.
          </p>
        ...
        {filterMode === "manual" ? (
          ...
          <div className="rounded-2xl border border-[var(--outline)] bg-[var(--surface)] p-4 text-sm text-[var(--muted)]">
            AI가 추정한 위험도 순으로 정렬되었습니다. 위험도가 높을수록 상단에 표시됩니다.
          </div>
        )}
      ...
      <section className="card-surface hidden h-full flex-col lg:flex">
        <div className="flex items-center justify-between border-b border-[var(--outline)] px-6 py-4">
          <div>
            <h3 className="text-lg font-semibold text-[var(--text)]">스레드 상세</h3>
          ...
          <Badge variant="muted">모더레이션</Badge>
        </div>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-[var(--outline)] bg-[var(--surface-2)] p-4 shadow-sm">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={detailRisk.variant}>{detailRisk.label}</Badge>
                </div>
              ...
              {selectedThread.aiSummary ? (
                <div className="rounded-2xl border border-[var(--outline)] bg-[var(--surface)] p-4 shadow-sm">
                  ...
                  {selectedThread.aiLabels?.length ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {selectedThread.aiLabels.map((label) => (
        ...
        {selectedThread ? (
          <div className="flex flex-wrap items-center gap-2 border-t border-[var(--outline)] bg-[var(--surface-2)] px-6 py-4">
            ...
            <Button variant="secondary" className="min-h-[44px]">
              <ShieldIcon className="mr-2 h-4 w-4" /> 보류
            </Button>
            <Button variant="secondary" className="min-h-[44px]">
              <ShieldIcon className="mr-2 h-4 w-4" /> 스팸 처리
            </Button>
            <Button variant="secondary" className="min-h-[44px]">
              <ShieldIcon className="mr-2 h-4 w-4" /> 게시 승인
            </Button>
            <Button variant="secondary" className="min-h-[44px]">
      ...
      <DrawerPanel
        ...
        actions={
          selectedThread ? (
            <div className="flex flex-wrap gap-2">
              <Button variant="primary" className="min-h-[44px] flex-1">
                답글
              </Button>
